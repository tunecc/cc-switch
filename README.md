...
设置页新增五个开关，可分别隐藏主页右侧入口：

- Skills（技能）
- MCP servers
- Sessions（会话历史）
- Prompts（提示词管理）
- 批量检测（顶栏批量连通性测试入口；关闭时若检测正在进行则停止。只作用于顶栏按钮，供应商卡片上的单个测试入口不受影响。未配置或旧数据缺少该字段时默认显示）
...
新建或编辑供应商时，可以从其他已启用应用里挑一个已有供应商，把配置预填进当前表单：

- 新建时，来源应用按钮出现在预设供应商区域；编辑时，同一组按钮出现在表单顶部。只列出已启用、至少有一个供应商、且不是当前应用的来源。
- 选中后预填名称、备注、官网链接、API Key、请求地址、默认模型名，以及目标应用支持的上游 API 格式。请求地址原样复制，不自动补 `/v1`。
- 只改当前表单，不直接保存；编辑模式下会覆盖上述已填字段，其余配置（图标、第二个官网链接、模型表等）保持原样。
- 来源与目标覆盖全部 10 个应用。默认模型名只在 Claude / Codex / Gemini 之间迁移。
- zh-CN / en / zh-TW / ja 四语言文案。
